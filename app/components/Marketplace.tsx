...
function AIEvalPanel({ job, delivHash }) {
  ...
  return (
    <div style={{background:"#E4EDE6",border:"1px solid #B8C8BC",borderRadius:12,padding:"14px 16px"}}>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <div style={{width:7,height:7,borderRadius:"50%",background:loading?"#B07A2A":"#5A8A5A"}}/>
          <span style={{fontSize:11.5,color:"#5A7A6A",fontWeight:500}}>AI Evaluation</span>
      ...
      {loading ? (
        <div>
          {[60,80,45].map((w,i)=><div key={i} className="shimmer" style={{height:12,borderRadius:4,background:"#B8C8BC",marginBottom:8,width:w+"%"}}/>)}
          <div style={{fontSize:12,color:"#7A6A52",marginTop:6}}>Analysing deliverable against requirements…</div>
        </div>
      ) : result && (
        <div className="fade-in">
          ...
          <div style={{marginBottom:12}}>
            {result.checks.map((c,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:9,padding:"6px 0",borderBottom:"1px solid #D8D0C0"}}>
          ...
          {/* Overall */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#EDE8DC",borderRadius:8,padding:"10px 12px"}}>
            <div>
            ...
            <div style={{textAlign:"right"}}>
              ...
              <div style={{fontFamily:"'Playfair Display',serif",fontWeight:700,fontSize:14,color:result.recommend==="APPROVE"?"#5A8A5A":"#A85440",background:result.recommend==="APPROVE"?"#E6EEE3":"#F5E4E0",border:`1px solid ${result.recommend==="APPROVE"?"#B8CDB4":"#D8B8AC"}`,padding:"4px 10px",borderRadius:5}}>
                {result.recommend}
              </div>
            </div>
          </div>
...
function CompleteJobModal({ job, deliverableMap, onClose, onCompleted }) {
  ...
  if (step==="done") return (
    <div className="slide-up" style={{background:"#F5F0E8",border:"1px solid #C8BFA8",borderRadius:16,padding:26,width:460,maxWidth:"94vw"}}>
      ...
      <div style={{background:"#EDE8DC",border:"1px solid #C8BFA8",borderRadius:9,padding:"10px 12px",marginBottom:13}}>
        ...
        <div style={{fontSize:11,color:"#7A6A52",marginTop:3}}>AgenticCommerce · complete(uint256,bytes32,bytes)</div>
      </div>
  ...
  return (
    ...
    >
      ...
      {/* USDC release */}
      <div style={{background:"#E6EEE3",border:"1px solid #B8CDB4",borderRadius:10,padding:"12px 14px",marginBottom:14}}>
        <div style={{fontSize:9.5,color:"#5A8A5A",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:.5,marginBottom:6}}>ESCROW RELEASE</div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:13,color:"#5A8A5A"}}>To {job.provider?.name||"provider"}</span>
          <span style={{fontFamily:"'IBM Plex Mono',monospace",color:"#5A8A5A",fontSize:16,fontWeight:700}}>{job.budget} USDC</span>
        </div>
        <div style={{fontSize:11,color:"#5A8A5A",marginTop:4}}>Released from escrow on Arc · immediate settlement</div>
      </div>
      ...
      <div style={{background:"#EDE8DC",border:"1px solid #C8BFA8",borderRadius:9,padding:"10px 12px",marginBottom:16}}>
        ...
        {[{l:"Contract",v:"AgenticCommerce (ERC-8183)"},{l:"Function",v:"complete(uint256,bytes32,bytes)"},{l:"Job ID",v:`#${job.id}`},{l:"reasonHash",v:`${reasonHash.slice(0,18)}…`},{l:"Est. fee",v:"~0.006 USDC"}].map(r=><div key={r.l} style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12,color:"#7A6A52"}}>{r.l}</span><span style={{fontSize:12,color:"#6A5E4A",fontFamily:"'IBM Plex Mono',monospace"}}>{r.v}</span></div>)}
      </div>
...
function RejectJobModal({ job, onClose, onRejected }) {
  ...
  if (step==="done") return (
    <div className="slide-up" style={{background:"#F5F0E8",border:"1px solid #C8BFA8",borderRadius:16,padding:26,width:440,maxWidth:"94vw"}}>
      ...
      <div style={{background:"#EDE8DC",border:"1px solid #C8BFA8",borderRadius:9,padding:"10px 12px",marginBottom:18}}>
        <div style={{fontSize:9.5,color:"#7A6A52",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:.5,marginBottom:4}}>TRANSACTION</div>
        <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10.5,color:"#6A5E4A",wordBreak:"break-all"}}>{txHash}</div>
        <div style={{fontSize:11,color:"#7A6A52",marginTop:3}}>AgenticCommerce · reject(uint256,bytes32,bytes)</div>
      </div>
      <div style={{background:"#F5E4E0",border:"1px solid #D8B8AC",borderRadius:9,padding:"10px 12px",marginBottom:18}}>
        <div style={{fontSize:11.5,color:"#A85440",lineHeight:1.6}}>The rejection reason is stored on-chain as a bytes32 hash — an immutable audit trail for both parties. The agent can query it via <span style={{color:"#A85440",fontFamily:"'IBM Plex Mono',monospace"}}>getJob({job.id})</span>.</div>
      </div>
  ...
  return (
    <div className="slide-up" style={{background:"#F5F0E8",border:"1px solid #C8BFA8",borderRadius:16,padding:26,width:440,maxWidth:"94vw"}}>
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:18}}>
        <div>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:16,fontWeight:700,color:"#A85440",marginBottom:3}}>Reject deliverable</div>
          <div style={{fontSize:12,color:"#7A6A52"}}>{job.title.slice(0,46)}{job.title.length>46?"…":""}</div>
        </div>
      ...
      <div style={{background:"#F5E4E0",border:"1px solid #D8B8AC",borderRadius:10,padding:"11px 13px",marginBottom:14}}>
        <div style={{fontSize:9.5,color:"#A85440",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:.5,marginBottom:4}}>ESCROW REFUND</div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:13,color:"#A85440"}}>Returned to your wallet</span>
      ...
      <div style={{background:"#EDE8DC",border:"1px solid #C8BFA8",borderRadius:9,padding:"10px 12px",marginBottom:14}}>
        {[{l:"Contract",v:"AgenticCommerce (ERC-8183)"},{l:"Function",v:"reject(uint256,bytes32,bytes)"},{l:"Job ID",v:`#${job.id}`},{l:"reasonHash",v:`${reasonHash.slice(0,18)}…`}].map(r=><div key={r.l} style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12,color:"#7A6A52"}}>{r.l}</span><span style={{fontSize:12,color:"#6A5E4A",fontFamily:"'IBM Plex Mono',monospace"}}>{r.v}</span></div>)}
      </div>
...
function EvaluationDashboard({ queue, deliverableMap, completedJobs, rejectedJobs, onComplete, onReject, isMobile }) {
  ...
  const evalTag = (ev) => isAIEval(ev)
    ? <span style={{fontSize:10,padding:"2px 6px",borderRadius:3,background:"#E4EDE6",color:"#5A7A6A",border:"1px solid #B8C8BC"}}>AI</span>
          ...
          return (
            <div style={{maxWidth:620}} className="fade-in">
              ...
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:18}}>
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                    ...
                    {isAIEval(sel.evaluator)&&<span style={{fontSize:10.5,padding:"2px 7px",borderRadius:3,background:"#E4EDE6",color:"#5A7A6A",border:"1px solid #B8C8BC"}}>AI Evaluator</span>}
                  </div>
              ...
              <div style={{background:"#EDE8DC",border:"1px solid #C8BFA8",borderRadius:10,padding:"12px 14px",marginBottom:14}}>
                ...
                {deliv ? <>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:7}}>
                    <span style={{fontSize:10,padding:"2px 6px",borderRadius:3,background:"#E4EDE6",color:"#5A7A6A",border:"1px solid #B8C8BC"}}>{deliv.dtype||"ipfs"}</span>
                    <a 
                      href={deliv.value.startsWith('ipfs://') 
                        ? `https://gateway.pinata.cloud/ipfs/${deliv.value.replace('ipfs://', '')}` 
                        : deliv.value}
              ...
              {resolved&&<div style={{background:isComp?"#E6EEE3":"#F5E4E0",border:`1px solid ${isComp?"#B8CDB4":"#D8B8AC"}`,borderRadius:10,padding:"13px 15px",marginBottom:18}}>
                ...
                <div style={{fontSize:12,color:isComp?"#5A8A5A":"#A85440",lineHeight:1.6}}>This job has been {isComp?"completed and settled on-chain":"rejected with reason stored on-chain"}.</div>
              </div>}
                    ...
                    {arbitrationResult.strengths?.length > 0 && (
                      <div style={{marginBottom:8}}>
                        ...
                        {arbitrationResult.strengths.map((s:string,i:number)=>(
                          <div key={i} style={{fontSize:12,color:'#5A8A5A',marginBottom:2}}>✓ {s}</div>
                        ))}
                      </div>
                    )}
                    {arbitrationResult.weaknesses?.length > 0 && (
                      <div style={{marginBottom:10}}>
                        ...
                        {arbitrationResult.weaknesses.map((w:string,i:number)=>(
                          <div key={i} style={{fontSize:12,color:'#A85440',marginBottom:2}}>✗ {w}</div>
                        ))}
                      </div>
                    )}
...
onSubmit({ score, tag, notes, txHash: tx, agentId, jobId: job.id })
  ...
  if(step==="done")return(<div className="slide-up" style={{background:"#F5F0E8",border:"1px solid #C8BFA8",borderRadius:16,padding:26,width:420,maxWidth:"94vw"}}>
    ...
    <div style={{background:"#EDE8DC",border:"1px solid #C8BFA8",borderRadius:9,padding:"10px 12px",marginBottom:12}}><div style={{fontSize:9.5,color:"#7A6A52",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:.5,marginBottom:3}}>TRANSACTION</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10.5,color:"#6A5E4A",wordBreak:"break-all"}}>{txHash}</div><div style={{fontSize:11,color:"#7A6A52",marginTop:3}}>ReputationRegistry · giveFeedback()</div></div>
    <div style={{background:"#E6EEE3",border:"1px solid #B8CDB4",borderRadius:9,padding:"9px 12px",marginBottom:16}}><div style={{fontSize:11.5,color:"#5A8A5A",lineHeight:1.6}}>Per ERC-8004, this was recorded by your wallet — not the agent's. Now part of {job.provider?.name}'s permanent onchain reputation.</div></div>
    <button className="btn-pri" onClick={onClose} style={{width:"100%",padding:"11px",borderRadius:9,background:"#1A1712",color:"#F5F0E8",border:"none",fontSize:13.5,fontFamily:"'Playfair Display',serif",fontWeight:600,cursor:"pointer"}}>Done</button>
  </div>);
  ...
  return(<div className="slide-up" style={{background:"#F5F0E8",border:"1px solid #C8BFA8",borderRadius:16,padding:26,width:420,maxWidth:"94vw"}}>
    ...
    <div style={{background:"#EDE8DC",border:"1px solid #C8BFA8",borderRadius:9,padding:"9px 11px",marginBottom:12}}>{[{l:"Agent ID",v:`#${agentId}`},{l:"Score",v:`${score} / int128`},{l:"Tag",v:tag},{l:"feedbackHash",v:`${fh.slice(0,18)}…`}].map(r=><div key={r.l} style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12,color:"#7A6A52"}}>{r.l}</span><span style={{fontSize:12,color:"#6A5E4A",fontFamily:"'IBM Plex Mono',monospace"}}>{r.v}</span></div>)}</div>
    <div style={{background:"#F5EBD8",border:"1px solid #D8C4A0",borderRadius:8,padding:"8px 11px",marginBottom:13,display:"flex",gap:8,alignItems:"flex-start"}}><span style={{color:"#B07A2A",fontSize:12,flexShrink:0,marginTop:1}}>⚠</span><div style={{fontSize:11.5,color:"#B07A2A",lineHeight:1.6}}>Your wallet must not own Agent #{agentId}. ERC-8004 rejects self-attested reputation.</div></div>
    <div style={{display:"flex",gap:10}}>
...
function SubmitDeliverableModal({ job, myAgent, onClose, onSubmit }) {
  ...
  if(step==="done"&&result)return(<div className="slide-up" style={{background:"#F5F0E8",border:"1px solid #C8BFA8",borderRadius:16,padding:26,width:440,maxWidth:"94vw"}}>
    ...
    <div style={{background:"#EDE8DC",border:"1px solid #C8BFA8",borderRadius:9,padding:"10px 12px",marginBottom:13}}><div style={{fontSize:9.5,color:"#7A6A52",fontFamily:"'IBM Plex Mono',monospace",letterSpacing:.5,marginBottom:3}}>TRANSACTION</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10.5,color:"#6A5E4A",wordBreak:"break-all"}}>{result.txHash}</div><div style={{fontSize:11,color:"#7A6A52",marginTop:3}}>AgenticCommerce · submit(uint256,bytes32,bytes)</div></div>
    <button className="btn-pri" onClick={onClose} style={{width:"100%",padding:"11px",borderRadius:9,background:"#1A1712",color:"#F5F0E8",border:"none",fontSize:13.5,fontFamily:"'Playfair Display',serif",fontWeight:600,cursor:"pointer"}}>Done</button>
  </div>);
...
const USDC_ABI = [
  ...
  if(regStep==="done"&&agent)return(<div style={{padding:"26px 30px",maxWidth:500}} className="fade-in">
    ...
    <div style={{background:"#E6EEE3",border:"1px solid #B8CDB4",borderRadius:9,padding:"9px 12px",marginBottom:16}}><div style={{fontSize:11.5,color:"#5A8A5A",lineHeight:1.6}}>ERC-8004 prevents self-reporting. Reputation is recorded by clients and validators after you complete work.</div></div>
...
function AgentDashboard({ agent, feedbackHistory, activeJobs, deliverableMap, onBrowseJobs, onSubmitDeliverable }) {
  ...
  return (<div style={{padding:"24px 28px",maxWidth:620}}>
    ...
    <div style={{background:"#E6EEE3",border:"1px solid #B8CDB4",borderRadius:9,padding:"9px 12px",marginBottom:16}}><div style={{display:"flex",gap:8,alignItems:"flex-start"}}><span style={{fontSize:13,flexShrink:0}}>⬡</span><div style={{fontSize:11.5,color:"#5A8A5A",lineHeight:1.6}}>Reputation is recorded by job clients — not by you. This is enforced at the contract level by ERC-8004.</div></div></div>
...
export function Marketplace() {
  ...
  return (
    <div style={{fontFamily:"'Inter',system-ui,sans-serif",background:"#F5F0E8",color:"#1A1712",minHeight:"100vh",display:"flex",flexDirection:"column",position:"relative"}}>
      <style>{CSS}</style>

      {/* ── MODALS ── */}
      {/* Toast notification */}
      {toast && (
        <div className="fade-in" style={{position:'fixed',bottom:isMobile?80:24,right:24,zIndex:300,background:toast.type==='error'?'#F5E4E0':toast.type==='success'?'#E6EEE3':'#FDFBF6',border:`1px solid ${toast.type==='error'?'#D8B8AC':toast.type==='success'?'#B8CDB4':'#C8BFA8'}`,borderRadius:12,padding:'12px 16px',display:'flex',alignItems:'center',gap:10,maxWidth:320,boxShadow:'0 4px 16px rgba(26,23,18,0.12)'}}>
          ...
          <span style={{fontSize:13,color:toast.type==='error'?'#A85440':toast.type==='success'?'#5A8A5A':'#1A1712',fontFamily:"'Inter', sans-serif",lineHeight:1.4}}>{toast.message}</span>
        </div>
      )}
      {anyModal&&<div style={{position:"fixed",inset:0,background:"rgba(26,23,18,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
          ...
          <div style={{
            ...
            {[
              ...
              <button key={item.id}
                ...
                <span style={{ fontSize: 10, fontFamily: "'Inter', sans-serif" }}>{item.label}</span>
              </button>
            ))}
            <button
              ...
              <span style={{ fontSize: 10, fontFamily: "'Inter', sans-serif" }}>Post</span>
            </button>
          </div>
        )}
        ...
        <main style={{flex:1,overflow:"auto",display:"flex",flexDirection:"column",paddingBottom:isMobile?60:0}}>
          ...
          {showPost&&<div style={{padding:"24px 28px",maxWidth:560}}>
            ...
            {postDone?<div style={{textAlign:"center",padding:"44px 24px",background:"#E6EEE3",borderRadius:14,border:"1px solid #B8CDB4"}}><div style={{fontSize:30,marginBottom:10,color:"#5A8A5A"}}>✓</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,color:"#5A8A5A",marginBottom:4}}>Job posted to Arc</div><button className="btn-pri" onClick={()=>{setPostDone(false);setShowPost(false);setForm({title:"",category:"Writing",budget:"",deadline:"",description:"",evaluator:"Manual review"});}} style={{marginTop:14,padding:"9px 20px",borderRadius:8,background:"#1A1712",color:"#F5F0E8",border:"none",fontSize:13,fontFamily:"'Playfair Display',serif",fontWeight:600,cursor:"pointer"}}>Back to Jobs</button></div>
